...
class Videollama2MetaForCausalLM(ABC):
    ...
    def encode_images_or_videos(self, images):
        ...
        data_batch = torch.stack(data_batch, dim=0) #[1,8,3,384,384]

        assert len(data_batch.size()) == 5
        batch_size = data_batch.size(0)

        frames = einops.rearrange(data_batch, 'b t c h w -> (b t) c h w') #8,3,384,384
        frames_features = self.get_model().get_vision_tower()(frames) #8,729,1152
        frames_features = einops.rearrange(frames_features, '(b t) n h -> b t n h', b = batch_size) #[1,8,729,1152]

        return self.temporal_aggregator(frames_features) #[1,676,3584]
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            while mm_token_indices.numel() > 0:
                ...
                start_point = int(self.get_model().embed_tokens(cur_input_ids[:mm_token_start]).size(0)) 
                cur_new_input_embeds.append(cur_mm_features)
     
                last_point = start_point+int(cur_mm_features.size(0))
                if labels is not None:
        ...
        return None, attention_mask, past_key_values, new_input_embeds, new_labels, start_point, last_point
