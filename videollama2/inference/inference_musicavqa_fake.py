...
class AVQADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Replace each placeholder (in the order found) with the next value in templ_values
        for i, placeholder in enumerate(placeholders):
            if "," in sample["templ_values"][2:-2]:
                replace = sample["templ_values"].split(",")
                text = text.replace(placeholder, replace[i].replace('"', "").replace("[", "").replace("]", ""), 1)
            else:
                text = text.replace(placeholder, sample["templ_values"][2:-2], 1)
        video_path = "/mnt/lynx1/datasets/AVQA/MUSIC-AVQA-videos-Synthetic/" +sample["video_id"]+".mp4"
...
def run_inference(args):
    ...
    dataset = AVQADataset(gt_questions, processor['video'], processor["audio"])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--video-folder', help='Directory containing video files.', default="/mnt/lynx1/datasets/AVQA/MUSIC-AVQA-videos-Synthetic")
    parser.add_argument('--question-file', help='Path to the ground truth file containing question.', default="./json/avqa-test_fake.json")
    parser.add_argument('--answer-file', help='Path to the ground truth file containing answers.', required=False)
    parser.add_argument('--output-file', help='Directory to save the model results JSON.', default="./MUSICFAKE.json") 
    ...
    parser.add_argument("--dataset", type=str, default="AVQA")
